...
// Theme functions.
// =============================================================================
define("WHITELABEL_URL", get_template_directory_uri());
define("WHITELABEL_PATH", get_template_directory());
require_once(WHITELABEL_PATH . "/inc/class-tgm-plugin-activation.php");
add_theme_support('post-thumbnails');
function whitelabel_register_required_plugins() {
	$plugins = array(
        array(
            'name'               => esc_html__('Slider Revolution',"whitelabel"),
            'slug'               => 'revslider',
            'source'             => WHITELABEL_PATH . '/inc/revslider.zip',
            'required'           => true,
            'version'            => '',
        ...
        array(
			'name'               => esc_html__('Hybrid Composer',"whitelabel"),
			'slug'               => 'hybrid-composer',
			'source'             => WHITELABEL_PATH . '/inc/hybrid-composer.zip',
			'required'           => true,
			'version'            => '',
			'force_activation'   => false,
			'force_deactivation' => false,
...
add_action( 'tgmpa_register', 'whitelabel_register_required_plugins' );
if (!isset( $content_width )) $content_width = 1200;
add_theme_support( 'automatic-feed-links' );
add_theme_support('title-tag');
function whitelabel_enqueue_front_end_script() {
    if (!defined("HC_PLUGIN_PATH")) {
        wp_enqueue_style("bootstrap", WHITELABEL_URL . "/css/bootstrap/css/bootstrap.css", array(), "1.0", "all");
        wp_enqueue_script("bootstrap",  WHITELABEL_URL . '/css/bootstrap/js/bootstrap.min.js', array("jquery"), "1.0", true);
        wp_enqueue_script("hc_script",  WHITELABEL_URL . '/inc/default.js', array(), "1.0",true);
        wp_enqueue_style("hc_style", WHITELABEL_URL . "/style.css", array(), "1.0", "all");
        wp_enqueue_style("hc_icons", WHITELABEL_URL . "/inc/icons/icons.css", array(), "1.0", "all");
        wp_enqueue_style("hc_css_skin", WHITELABEL_URL . "/skin.css", array(), "1.0", "all");
    }
    if (is_singular() && comments_open() && (get_option('thread_comments') == 1)) wp_enqueue_script('comment-reply');
}
add_action('wp_enqueue_scripts', 'whitelabel_enqueue_front_end_script');
...
function whitelabel_sidebar() {
    add_meta_box('whitelabel_sidebar', 'Sidebars', function () {
...
<select data-hc-setting="sidebars" id="sidebars-menu" name="sidebars-menu">
    <option value="" <?php if ($sidebar == "") echo "selected" ?>><?php esc_attr_e("None","whitelabel") ?></option>
    <option value="right" <?php if ($sidebar == "right") echo "selected" ?>><?php esc_attr_e("Right","whitelabel") ?></option>
    <option value="left" <?php if ($sidebar == "left") echo "selected" ?>><?php esc_attr_e("Left","whitelabel") ?></option>
    <option value="right-left" <?php if ($sidebar == "right-left") echo "selected" ?>><?php esc_attr_e("Right and left","whitelabel") ?></option>
</select>
<?php
...
add_action('add_meta_boxes', 'whitelabel_sidebar');

function whitelabel_theme_add_editor_styles() {
    add_editor_style(WHITELABEL_URL . "/css/tiny-mce.css");
}
add_action('admin_init', 'whitelabel_theme_add_editor_styles');

//MENU
function whitelabel_init_menus() {
    register_nav_menus(
          array(
            'header-menu' => esc_html__('Header Menu',"whitelabel"),
            'footer-menu' => esc_html__('Footer Menu',"whitelabel"),
            'extra-menu' => esc_html__('Top mini Menu',"whitelabel")
          )
    );
}
add_action('after_setup_theme', 'whitelabel_init_menus');
...
function whitelabel_widgets_init() {
    register_sidebar( array(
        'name'          => esc_html__('Right Sidebar',"whitelabel"),
        'id'            => 'right_side_bar',
        'description'   => esc_html__('Global sidebar for pages, enable it on single page.',"whitelabel"),
    ...
    register_sidebar(array(
        'name'          => esc_html__('Left Sidebar',"whitelabel"),
        'id'            => 'left_side_bar',
        'description'   => esc_html__('Global sidebar for pages, enable it on single page.',"whitelabel"),
        'before_widget' => '<div class="list-group list-blog">',
        'after_widget'  => '</div>',
    ...
    if (class_exists('WooCommerce')) {
        register_sidebar(array(
           'name'          => esc_html__('Woocommerce Shop Sidebar Left',"whitelabel"),
           'id'            => 'woocommerce_shop_left_side_bar',
           'description'   => esc_html__('Shop sidebar, enable it on Theme options > List Post Types',"whitelabel"),
           'before_widget' => '<div class="list-group list-blog">',
           'after_widget'  => '</div>',
           'before_title'  => '<p class="list-group-item active">',
           'after_title'   => '</p>',
        ...
        register_sidebar(array(
          'name'          => esc_html__('Woocommerce Shop Sidebar Right',"whitelabel"),
        ...
        register_sidebar(array(
          'name'          => esc_html__('Woocommerce Item Sidebar Left',"whitelabel"),
          'id'            => 'woocommerce_single_left_side_bar',
          'description'   => esc_html__('Single product sidebar, enable it on Theme options > List Post Types',"whitelabel"),
          'before_widget' => '<div class="list-group list-blog">',
          'after_widget'  => '</div>',
          'before_title'  => '<p class="list-group-item active">',
          'after_title'   => '</p>',
        ...
        register_sidebar(array(
           'name'          => esc_html__('Woocommerce Item Sidebar Right',"whitelabel"),
...
//MAIN CONTENT
function whitelabel_get_post_info() {
    $txt = "";
    ...
    if ( ! empty( $categories ) ) {
        foreach( $categories as $category ) {
            $output .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" alt="' . esc_attr( sprintf( __( 'View all posts in %s', 'whitelabel' ), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>' . $separator;
        }
...
function whitelabel_the_content() {
    ...
    <div class="header-base search-results-header">
        <div class="container">
            <div class="row">
                <div class="col-md-9">
                    <div class="title-base text-left">
                        <h1><?php echo esc_html__("Search results for ","whitelabel") . get_search_query() ?></h1>
                        <p><?php esc_attr_e("Search results for ","whitelabel"); echo '<b><i>' . get_search_query() . '</i></b>'; ?></p>
                    </div>
                </div>
            </div>
        </div>
...
?>                         <h3>
            ...
            if (defined("HC_PLUGIN_PATH")) {
                echo hc_get_the_excerpt($wp_query->post->post_excerpt);
                            ...
                            <a class="circle-button btn btn-sm" href="<?php echo esc_url($link) ?>">
                                <?php esc_attr_e("Read more","whitelabel") ?>
                            </a>
                        </div>
                    </div>
                </div>
    ...
    } else echo "<h2 class='no-search-results'>" . esc_html__("No results found ...","whitelabel") . "</h2><hr class='space row-500' />";
                ?>
            </div>
        </div>
    </div>
...
function whitelabel_set_default_menu() {
...
function whitelabel_default_blog() {
    global $wp_query;
    if (have_posts()) {
        while (have_posts()) {
            the_post(); 
   ...
   <div class="advs-box advs-box-side default-box <?php if (is_sticky()) echo "sticky-post"; ?>" data-anima="fade-left" data-trigger="hover">
    <div class="row">
        <div class="col-md-12">
          ...
          <?php echo whitelabel_get_post_info(); ?>
